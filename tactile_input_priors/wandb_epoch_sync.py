...
def _pending_payloads(queue_dir: Path) -> list[Path]:
    return sorted(queue_dir.glob("epoch_*.json"))
...
def _acquire_lock(lock_path: Path, wait_seconds: float):
...
def _upload_pending(args) -> int:
    queue_dir = Path(args.queue_dir)
    pending = _pending_payloads(queue_dir)
    if not pending:
        return 0
    ...
    config = _load_json(Path(args.config)) if args.config else {}
    run = None
    uploaded: list[Path] = []
    ...
    try:
        ...
        for payload_path in pending:
            ...
            metrics.setdefault("trainer/global_step", float(payload["global_step"]))
            run.log(metrics, step=int(payload["global_step"]), commit=True)
...
def main() -> None:
    ...
    args.queue_dir = str(queue_dir)
    if args.config:
        args.config = str(Path(args.config).expanduser().resolve())

    lock_handle = _acquire_lock(queue_dir / ".upload.lock", args.lock_wait_seconds)
    if lock_handle is None:
        print("Another WandB epoch uploader owns the queue lock.", flush=True)
        return

    try:
        for attempt in range(max(int(args.retries), 1)):
            try:
                while _pending_payloads(queue_dir):
                    count = _upload_pending(args)
                    print(
                        f"Uploaded {count} epoch payload(s) to WandB run {args.run_id}.",
                        flush=True,
                    )
                return
            except Exception as exc:
                print(
                    f"WandB epoch upload attempt {attempt + 1}/{args.retries} failed: "
