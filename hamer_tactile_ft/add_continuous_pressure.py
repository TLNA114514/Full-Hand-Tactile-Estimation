...
def load_dependencies():
    ...
    # 3. Load layout
    # 使用 handLayoutNewest_meshid.json 或 lowres 版本。优先尝试 lowres。
    ...
    try:
        with h5py.File(filepath, "r+") as f:
            ...
            for demo_name in data_group.keys():
                ...
                if "right_pressure" in demo and "right_pressure_continuous" not in demo:
                    needs_processing = True
                if "left_pressure" in demo and "left_pressure_continuous" not in demo:
                    needs_processing = True
                    
            if not needs_processing:
                return  # Silently skip if fully processed, or print if you prefer
    ...
    for mf in tqdm(meta_files, desc="Processing extracted dataset"):
        try:
            ...
            for key in ["right_pressure", "left_pressure"]:
                if key in hdf5_data:
                    has_pressure = True
                    if f"{key}_continuous" not in hdf5_data:
            ...
            if modified:
                ...
                print(f"\n[Skipped] No valid pressure data found in {mf}")
        except Exception as e:
            print(f"Error processing {mf}: {e}")

def main():
    print("Loading dependencies (MANO mesh, auto_calibrated_palm_faces, handLayoutNewest_meshid)...")
    ...
    # 1. 修改 /data/jiangrui/OpenTouch Data/data/ 下的 .h5 / .hdf5
    ...
    # 2. 修改 /data/jiangrui/OpenTouch Data/extracted_dataset/ 下的文件
