...
def discover_sequences(raw_root, gaussian_npz_name, max_sequences=None, split_name=None):
    raw_root = Path(raw_root)
    sequences = []
    for data_json in sorted(raw_root.rglob("data.json")):
...
def main():
    ...
    parser.add_argument(
        "--cache_dir",
        type=str,
        default=os.path.join(base_dir, "hamer_tactile_ft/egotactile_bboxes_cache"),
        help="Per-sequence bbox cache directory.",
    )
    parser.add_argument(
        "--bbox_json",
        type=str,
        default=os.path.join(base_dir, "hamer_tactile_ft/egotactile_all_bboxes.json"),
