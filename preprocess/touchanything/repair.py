...
def main():
    if "--gaussian" in sys.argv[1:]:
        if "-h" in sys.argv[1:] or "--help" in sys.argv[1:]:
            print(
                "usage: repair.py --gaussian [--gpu GPU] [--workers_per_gpu N] "
                "[--ta_dir DIR] [--clip_dir DIR] [--only_missing_continuous] "
                "[--check_workers N] [--repo_root DIR] "
                "[--left_grid_transform {none,flip_lr,flip_ud,rot180}] "
                "[--right_grid_transform {none,flip_lr,flip_ud,rot180}]"
            )
            print()
            print("Run local TouchAnything Gaussian/continuous pressure generation.")
            print()
            print("Common example:")
            print(
                "  python preprocess/touchanything/repair.py --gaussian "
                "--gpu 0,1,2,3,4,5,6,7 --ta_dir /data1/jiangrui/EgoTouch "
                "--only_missing_continuous --check_workers 128 --workers_per_gpu 4"
            )
            return
        sys.argv = [sys.argv[0], *[arg for arg in sys.argv[1:] if arg != "--gaussian"]]
        import _gaussian
    ...
    parser.add_argument(
        "--gaussian",
        action="store_true",
        help="Run local TouchAnything Gaussian/continuous pressure generation instead of NPZ repair.",
    )
