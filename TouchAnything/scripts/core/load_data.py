...
def export_tactile_voronoi(
    ...
    with h5py.File(file_path, "r") as src:
        ...
        for name in dataset_names:
            ...
            prev_map = None
            for idx, sample in enumerate(dset):
                # Prepare 16x16 pressure grid (reuses your helper)
...
def export_tactile_mano_ta(
    ...
    layout_json_path=None,         # <<< NEW: direct path to layout JSON file
):
    """
    ...
    TouchAnything version: supports 21x21 grid and [0,1] pressure range.
    """
    import os, json, cv2, numpy as np, h5py, trimesh, sys
    from matplotlib import cm
    from pathlib import Path
    
    # Add the local MANO visualization asset path so ``pyrenderer`` can be imported.
    _script_dir = Path(__file__).resolve().parent
    _project_root = _script_dir.parent.parent
    _mano_vis_candidates = [
        _script_dir.parent / 'tools' / 'mano_visualization',
        _project_root / 'third_party' / 'opentouch_visualization_subset' / 'opentouch' / 'preprocess',
        _project_root / 'ref' / 'opentouch' / 'preprocess',
    ]
    _mano_vis_root = next(
        (p for p in _mano_vis_candidates if p.exists()),
        _mano_vis_candidates[0],
    )
    if str(_mano_vis_root) not in sys.path:
        sys.path.insert(0, str(_mano_vis_root))
    ...
    def _load_layout_json():
        # If a direct path is provided, use it; otherwise search default locations
        if layout_json_path is not None:
            if not os.path.exists(layout_json_path):
                raise FileNotFoundError(f"Specified layout JSON not found: {layout_json_path}")
    ...
    def _render_pressure_mano(mano_vertices, mano_faces, renderer, pressure_grid, layout, erased_nodes, vmin, vmax, nbrs, dists):
        from collections import defaultdict
        # normalize pressure → [0,1]
        norm = ((pressure_grid - vmin) / max(vmax - vmin, 1e-6)).clip(0, 1)
        valid_nodes = {
        ...
        for nid, info in valid_nodes.items():
            # Support both "r-c" and "r,c" formats
            separator = ',' if ',' in nid else '-'
            r, c = map(int, nid.split(separator))
            val = float(norm[r, c])
        ...
        if vert_to_vals:
            ...
            # TouchAnything: keep unmapped vertices at 0 (no pressure) to avoid white regions.
        ...
        # min-max normalize (TouchAnything: 0=no pressure, 1=max pressure, no invert)
        # mn, mx = float(vert_vals.min()), float(vert_vals.max())
        # if mx > mn:
        #     vert_vals = (vert_vals - mn) / (mx - mn)
        # else:
        #     vert_vals[:] = 0.0
    ...
    with h5py.File(file_path, "r") as src:
        ...
        obj_path = _find_first_existing([
            "data/mano_right_neutral_subdiv.obj",
            str(_script_dir / "data" / "mano_right_neutral_subdiv.obj"),
            str(_script_dir / "scratch" / "mano_right_neutral_subdiv.obj"),
            str(_project_root / "src" / "resources" / "mano_right_neutral_subdiv.obj"),
            str(_scripts_root / "tools" / "mano_visualization" / "scratch" / "mano_right_neutral_subdiv.obj"),
            str(_project_root / "third_party" / "opentouch_visualization_subset" / "opentouch" / "preprocess" / "scratch" / "mano_right_neutral_subdiv.obj"),
            str(_project_root / "ref" / "opentouch" / "preprocess" / "scratch" / "mano_right_neutral_subdiv.obj"),
        ])
        if obj_path is None:
        ...
        for name in dataset_names:
            ...
            # Detect whether the current dataset belongs to the left hand.
            is_left_hand = "left" in name.lower()

            # gather all frames to set global vmin/vmax on valid nodes (21x21 for TouchAnything)
            vals = []
            for sample in dset:
                grid = _prepare_tactile_frame(sample, dset.attrs).astype(np.float32)
                if grid.shape != (21, 21):
                    grid = cv2.resize(grid, (21, 21), interpolation=cv2.INTER_AREA).astype(np.float32)
                vals.append(grid)
            stack = np.stack(vals, 0) if vals else np.zeros((0, 21, 21), np.float32)

            valid_mask = np.zeros((21, 21), dtype=bool)
            for nid in layout.keys():
            ...
            # TouchAnything data is already normalized to [0,1]; avoid recomputing vmin/vmax.
            # Use a fixed range to prevent a second normalization pass.
            vmin, vmax = 0.0, 1.0
            # if stack.size:
            #     vmin = float(stack[:, valid_mask].min())
            #     vmax = float(stack[:, valid_mask].max())
            ...
            for idx, grid in enumerate(vals):
                ...
                # Flip left-hand images horizontally.
                if is_left_hand:
                    img_bgr = cv2.flip(img_bgr, 1)  # 1 means horizontal flip.
                # save frame image
...
def export_pose_mano(
    ...
    _project_root = _scripts_root.parent
    _easymocap_root = _project_root / "third_party" / "EasyMocap"
    if _easymocap_root.exists() and str(_easymocap_root) not in sys.path:
        sys.path.insert(0, str(_easymocap_root))
    ...
    def _resolve_file(filename):
        _visual_asset_root_candidates = [
            _scripts_root / "tools" / "mano_visualization",
            _project_root / "third_party" / "opentouch_visualization_subset" / "opentouch",
            _project_root / "ref" / "opentouch",
        ]
        candidates = [
            _here / "data" / filename,
            _here / "scratch" / filename,
            _easymocap_root / "data" / "smplx" / filename,
            _here.parent / "EasyMocap" / "data" / "smplx" / filename,
        ]
        for root in _visual_asset_root_candidates:
            candidates.extend([
                root / "preprocess" / "data" / filename,
                root / "preprocess" / "scratch" / filename,
                root / "EasyMocap" / "data" / "smplx" / filename,
                root / filename,
                root / "scratch" / filename,
            ])
        candidates.extend([
            _project_root / "third_party" / "hamer" / "_DATA" / "data" / "mano" / filename,
            _project_root / "ref" / "mano_v1_2" / "models" / "mano" / filename,
            _project_root / "ref" / "mano_v1_2" / "models" / filename,
        ])
    ...
    def nudge_thumb_lateral_3j(arr, k_tip=0.08, k_ip=0.05, k_mcp=0.03):
        ...
        for t in range(T):
            ...
            arr[t] = c
        return arr
    # ---------------- pre-align keypoints to MANO canonical frame --------
    def _prealign_to_mano_canonical(arr, side):
        """Rotate hand keypoints so that the hand's local frame aligns with
        MANO_RIGHT canonical orientation (fingers +Y, palm-normal +Z, thumb +X).
        For the left hand, an additional X-flip is applied so that it can be
        fitted with MANO_RIGHT.  This keeps Rh small after fitting, so zeroing
        Rh preserves the correct finger articulation.

        arr: (T, 21, 3)   side: 'left' | 'right'
        Returns aligned copy with wrist centred at origin each frame.
        """
        if arr.ndim != 3 or arr.shape[1] != 21:
            return arr  # safety: skip malformed data

        # Compute alignment rotation from the *mean* pose (stable over noise)
        mean_pts = arr.mean(axis=0)                       # (21, 3)
        wrist = mean_pts[0].copy()
        pts = mean_pts - wrist

        # Up direction: wrist → middle-finger MCP (joint 9)
        up = pts[9].copy()
        ...
        # Lateral direction: pinky MCP (17) → index MCP (5)  (toward thumb)
        lateral = pts[5] - pts[17]
        lat_norm = np.linalg.norm(lateral)
        if lat_norm < 1e-8:
            return arr
        lateral = lateral / lat_norm

        # Palm-normal = lateral x up  (right-hand cross product)
        forward = np.cross(lateral, up)
        fw_norm = np.linalg.norm(forward)
        if fw_norm < 1e-8:
            return arr
        forward = forward / fw_norm

        # Re-orthogonalise lateral (keep up & forward exact)
        lateral = np.cross(up, forward)
        lateral = lateral / (np.linalg.norm(lateral) + 1e-8)

        # Rotation: world → canonical  (columns = local basis vectors)
        R_local = np.stack([lateral, up, forward], axis=1)   # 3x3
        R_align = R_local.T                                  # inverse

        # Apply to every frame (centre at wrist first)
        aligned = arr.copy()
        for t in range(aligned.shape[0]):
            aligned[t] = aligned[t] - aligned[t, 0:1]       # wrist → origin
            aligned[t] = (R_align @ aligned[t].T).T

        # For left hand: mirror X (handedness) AND negate Z (palm direction).
        # cross(lateral, up) gives palm-normal for left hand but anti-palm
        # for right hand.  Negating Z makes both hands share the same
        # convention so the same renderer rotation works for both.
        if side == 'left':
            aligned[:, :, 0] = -aligned[:, :, 0]
            # aligned[:, :, 2] = -aligned[:, :, 2]
    ...
    # -------------------------- one-time renderer ----------------------------
    class RenderOnce:
        def __init__(self, faces, image_size=(480,480), bg_rgb=(249, 235, 142), side='right'):
            W, H = image_size
            self.W, self.H = W, H
            self.scene = pyrender.Scene(bg_color=[*bg_rgb, 0], ambient_light=[0.25, 0.25, 0.25])
            # camera - reduced focal length for wider FOV to fit entire hand
            cam = pyrender.IntrinsicsCamera(fx=6000, fy=6000, cx=W/2, cy=H/2, zfar=1e12)
            T = np.eye(4); T[:3,3] = [0,0,2.2]  # moved camera back slightly
            ...
            # Rotation transform: both hands use Ry(90°)+Rx(-90°); the left hand is mirrored by image flipping.
            ...
            self.flip_h = (side == 'left')  # Flip the rendered left hand horizontally.
        ...
        def render(self, verts):
            ...
            # Center the vertices so the hand stays near the middle of the frame.
            v = v - v.mean(axis=0, keepdims=True)
            # Adaptive scaling so the hand occupies a fixed portion of the frame.
            extent = np.abs(v).max()
            if extent > 1e-6:
                v = v * (0.09 / extent)
            ...
            img = rgba[:, :, :3]
            if self.flip_h:
                img = img[:, ::-1, :]  # Horizontal flip for the left hand so the thumb moves from right to left.
            return img
        ...
        for name in dataset_names:
            ...
            if dpath in src and isinstance(src[dpath], h5py.Dataset):
                ...
                if arr.ndim == 2 and arr.shape[0] == 21:
                    arr = arr[None, ...]
                arr = _prealign_to_mano_canonical(arr, _pick_side(name))
    ...
    for name, seq in sequences.items():
        ...
        # Always fit with MANO_RIGHT (keypoints already share the same X flip) to avoid invalid poses from zeroed Rh.
        body_model, device = _load_mano_model(
            side='right', model_root=mano_model_root, use_cuda=use_cuda,
        ...
        rctx = RenderOnce(faces=faces, image_size=target_size, bg_rgb=background_color, side=side)
        ...
        for nf in range(T):
            ...
            if verts.ndim == 3:
                verts = verts[0]
            # Do not mirror left-hand vertices directly; flip the rendered image instead.
